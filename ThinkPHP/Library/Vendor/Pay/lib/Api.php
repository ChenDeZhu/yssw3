...
class WxPayApi
{
	...
	public static function myunifiedOrder($inputObj, $timeOut = 60)
	{
		$url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
		$inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
	...
	{		
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			//throw new WxPayException("网络请求超时！如果注册了，<br>请通过手机号码查询报名情况，完成支付报名!<br><a href='index.php'>去完成注册</a>");
			echo "网络请求超时！如果注册了，<br>请通过手机号码查询报名情况，完成支付报名!<br><a href='index.php'>去完成注册</a>";
		}
	}

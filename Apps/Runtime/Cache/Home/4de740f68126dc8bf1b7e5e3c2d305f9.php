...
<html>
	...
	<body>
		<div class="mui-content topws">
			...
			<div class="mui-input-row" style="margin: 10px 5px;">
					<textarea  rows="10" placeholder="请输入工作经历" id="content"><?php echo ($user["work_experience"]); ?></textarea>
				</div>
				<div class="mui-button-row">
					<button type="button" class="mui-btn mui-btn-primary" id="updatec">更新信息</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="mui-btn mui-btn-danger" onclick="window.history.back();">取消更新</button>
				</div>
		</div>
	...
	<script>
        ...
        mui('#updatec')[0].addEventListener('tap',function(){
        	...
        	postdate['work_experience'] = mui('#content')[0].value;
        	// postdate['content'] = document.getElementById('content').value;

        	mui.post("<?php echo U('user_work_experience');?>",postdate,function(res){
					if(res==1){
						mui.toast('修改成功');
						setTimeout("window.location.href='/index.php/User/update_members'",500);
					}else{

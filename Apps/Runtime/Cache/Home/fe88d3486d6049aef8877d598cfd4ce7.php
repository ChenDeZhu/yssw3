...
<header class="mui-bar mui-bar-nav">
	<div class="type">
		<ul>
			<?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Information/personnel',array('cid'=>$vo['cid']));?>" <?php if($cid == $vo['cid']): ?>class="no"<?php endif; ?>><li><?php echo ($vo["name"]); ?></li></a><?php endforeach; endif; else: echo "" ;endif; ?>
		</ul>
	</div>
</header>
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell mui-media">
			...
			<a class="mui-navigate-right" href="<?php echo U('Information/detail',array('id'=>$vo['id']));?>">
				...
				<div class="mui-media-body">
					<?php echo ($vo["title"]); ?>
					<div class="parameter">
						<dl><dt>信息类型</dt><dd><?php echo ($vo["tname"]); ?></dd></dl>
						<dl><dt>信息类别</dt><dd><?php echo ($vo["cname"]); ?></dd></dl>
						<dl><dt>发布日期</dt><dd><?php echo (date("Y年m月d日",$vo["updatetime"])); ?></dd></dl>
					</div>
				</div>
			</a>
...
<div class="mui-bar mui-bar-tab">
	...
	<a class="mui-tab-item mui-active" href="<?php echo U('information/personnel');?>"><span class="mui-icon mui-icon-person"></span><span class="mui-tab-label">人才</span></a>

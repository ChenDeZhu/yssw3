...
<header class="mui-bar mui-bar-nav">
	<div class="type">
		<ul>
			<?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Information/supply',array('cid'=>$vo['cid']));?>" <?php if($cid == $vo['cid']): ?>class="no"<?php endif; ?>><li><?php echo ($vo["name"]); ?></li></a><?php endforeach; endif; else: echo "" ;endif; ?>
		</ul>
	</div>
</header>
	...
	<ul class="mui-table-view mui-table-view-chevron productlb">
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell mui-media">
				...
				<a class="mui-navigate-right" href="<?php echo U('Information/detail',array('id'=>$vo['id']));?>">
					<img class="mui-media-object mui-pull-left" <?php if($vo[img] == null): ?>src="/Public/Home/Template/images/p-4.jpg"<?php else: ?>src="<?php echo ($vo['img']); ?>"<?php endif; ?>>
					<div class="mui-media-body">
						<?php echo ($vo["title"]); ?>
						<div class="parameter">
							<dl><dt>频道</dt><dd><?php echo ($vo["tname"]); ?></dd></dl>
							<dl><dt>类别</dt><dd><?php echo ($vo["cname"]); ?></dd></dl>
							<dl><dt>日期</dt><dd><?php echo (date("Y年m月d日",$vo["updatetime"])); ?></dd></dl>
						</div>
					</div>
				</a>
...
<div class="mui-bar mui-bar-tab">
	...
	<a class="mui-tab-item mui-active" href="<?php echo U('information/supply');?>"><span class="mui-icon mui-icon-compose"></span><span class="mui-tab-label">供求</span></a>
				...
				setTimeout(function() {
					...
					mui.post("<?php echo U('information/upRefresh');?>",{
							...
							if(res['status'] ==1){
								...
								li.innerHTML = '<span class="shijian">&yen; <b>'+res['data'][i]['price']+'</b></span><a class="mui-navigate-right" href="/index.php/information/detail/id/'+id+'"><div class="mui-media-body">'+res['data'][i]['title']+'<div class="parameter"><dl><dt>频道</dt><dd>'+res['data'][i]['tname']+'</dd></dl><dl><dt>类别</dt><dd>'+res['data'][i]['name']+'</dd></dl><dl><dt>日期</dt><dd>'+res['data'][i]['date']+'</dd></dl></div></div></a>';
